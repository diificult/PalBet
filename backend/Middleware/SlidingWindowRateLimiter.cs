...
namespace PalBet.Middleware
{
    ...
    ";
        ...
        private static string GetApiKey(HttpContext context)
        {
            ...
            var scheme = header.Scheme?.ToLowerInvariant() ?? string.Empty;
            var parameter = header.Parameter ?? string.Empty;
            return parameter;
        }

        public IEnumerable<RateLimitRules> GetApplicableRules(HttpContext context)
        {
            var limits = _config.GetSection("RedisRateLimits").Get<RateLimitRules[]>();
        ...
        private async Task<bool> IsLimited(IEnumerable<RateLimitRules> rules, string apiKey)
        {
        ...
        public async Task InvokeAsync(HttpContext httpContext)
        {
            ...
            // If endpoint allows anonymous access, skip rate limiting
